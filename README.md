...
| **64-way Shard Striping** | MurmurHash3 routes keys to independent shards — zero cross-shard lock contention |
...
| **Lazy Tombstone Deletion** | `DEL` is O(1) — sets `tombstoned` flag; physical erasure deferred to `TimingWheel` |
...
┌──────────────────▼───────────────────────────┐
│              Daemon Layer                    │
│   AofLogger (fsync/1s) + TimingWheel (sweep) │
└──────────────────────────────────────────────┘
```
...
| Key overlap | High (100 unique keys per worker — forces shard contention) |
